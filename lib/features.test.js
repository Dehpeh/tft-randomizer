/* What the feature detectors are actually checked against.
   ...
   5x7 font and a trait panel with n hexagons lit. That tests
   the geometry — one glyph, closed hole, ring balanced above and below; a
   hexagon brighter than the gap beneath it — and it tests nothing
...
(function (root, factory) {
  ...
  function panel(n, bg, bgSat) {
    ...
    const x0 = Math.round(T.x0 * W);
    const x1 = Math.round(T.x1 * W);
    /* Drawn at the same centres the sampler reads — a generator with its own
       idea of where a row is tests the generator, not the rule. */
    for (let row = 0; row < T.rows; row++) {
      const cy = T.y0 + row * T.pitch;
      const top = Math.round((cy - T.half) * H);
      const bot = Math.round((cy + T.half) * H);
      const lit = row < n;
      for (let y = top; y < bot; y++) {
        for (let x = x0; x < x1; x++) {
          const k = (y * W + x) * 4;
          if (lit) { data[k] = 205; data[k + 1] = 140; data[k + 2] = 55; }   // bronze
          else { data[k] = 42; data[k + 1] = 42; data[k + 2] = 44; }         // dark grey
        }
      }
    }
    return { width: W, height: H, data };
  ...
  return { run: run, plate: plate, panel: panel };
});
