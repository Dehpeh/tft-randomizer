...
(function (root, factory) {
  ...
  /* ---------- the trait panel ----------
     ...
     A fourth attempt was written and scored: judge each hexagon against the
     dimmest one in its own column, on the theory that a panel almost always
     carries inactive traits and would supply its own reference. It is worse. It
     answers 3 on all three labelled frames, including the two where the truth
     is 0, because the panel has a vertical gradient of its own and the upper
     rows sit above the floor whether or not anything is lit. Recorded so nobody
     spends an evening rediscovering it.

     What is left to try is reading the fraction instead of the colour. An
     inactive trait prints a fraction like 1 of 2; an active one prints its
     breakpoint list instead. The digit machinery in digits.js can tell those
     apart, and it does not care what tier anything is or how saturated it
     looks — which is the assumption all four colour attempts have died on.
     That is the approach with a reason to work, and it is not built.
