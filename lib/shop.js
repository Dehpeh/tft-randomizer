...
(function (root, factory) {
  ...
  const DEFAULT = {
    ...
    w: 0.099,       // card width
    /* Measured on a 1080p capture by scanning down a slot: the card art runs to
       0.964 and the name bar occupies 0.967 to 0.993. The first estimate started
       at 0.956 and so took in a strip of portrait, which is enough to push a
       1-cost over the saturation line and read it as a 3-cost. */
    barY: 0.967,    // the name bar: where the cost colour lives
    barH: 0.026,
  ...
  const PALETTE = [
    ...
    /* 1 and 3 share a hue band and are told apart by saturation alone. The line
       sits at 0.52 because that is the middle of the gap on both captures that
       have been measured: at 1080p 1-costs top out at 0.47 and 3-costs start at
       0.56, and at 360p it is 0.44 against 0.54. The first attempt at 0.48 was
       inside a hundredth of a real 1-cost. */
    { cost: 3, hue: [165, 245], minSat: 0.52, name: 'blue' },
  ...
  /* On by default now.
     ...
     It earned it the way everything here has to: scored on two full games at
     the resolution people actually play at. Costs read correctly on every
     labelled frame across both, a purchase was confirmed against the recording
     frame by frame, and the three things that made it lie — the sell bar, the
     fade through combat, and an overlay covering a slot — are each pinned in
     lib/shop.test.js.

     It is also quiet by construction, which is what makes turning it on safe
     while a tournament is running. The detector reports what the shop did; the
     proctor only writes a note when one of the player's own rolled
     restrictions is about the shop. A player with no shop restriction sees
     nothing from it at all.

     Setting the key to '0' still turns it off for a browser. */
  function on() {
    try {
