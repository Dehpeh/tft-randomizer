...
(function (root, factory) {
  ...
  const MINOR = [
    { id: 'mn-afk',       family: 'afk',      text: 'AFK 1 round every stage', detail: { label: 'Round', options: ['1st round', '2nd round', '3rd round', '4th round'] } },
    { id: 'mn-shoplock',  family: 'shop',     text: 'Lock 1 shop space, 1 round every stage', detail: { label: 'Shop slot', options: ['1st slot', '2nd slot', '3rd slot', '4th slot', '5th slot'] } },
    { id: 'mn-augment',   family: 'augment',  text: '1 augment is chosen randomly', detail: { label: 'Augment', options: ['1st (2-1)', '2nd (3-2)', '3rd (4-2)'] } },
    { id: 'mn-costban',   family: 'costban',  text: '5 costs banned until stage 5' },
    { id: 'mn-carousel',  family: 'carousel', text: 'Carousel pick banned for 2 stages', detail: { label: 'Stages', options: ['stages 2 and 3', 'stages 3 and 4', 'stages 4 and 5', 'stages 5 and 6'] } },
  ...
  const MAJOR = [
    ...
    { id: 'mj-shoplock',  family: 'shop',     text: 'Lock 1 shop space', detail: { label: 'Shop slot', options: ['1st slot', '2nd slot', '3rd slot', '4th slot', '5th slot'] } },
    { id: 'mj-afk',       family: 'afk',      text: 'AFK for a whole stage', detail: { label: 'Stage', options: ['stage 2', 'stage 3', 'stage 4', 'stage 5', 'stage 6'] } },
  ...
  function toPick(entry, tier, rerolls, rng) {
    ...
    if (entry.detail) {
      pick.detail = {
        ...
        value: entry.detail.options[Math.floor(rng() * entry.detail.options.length)],
      };
    }
    return pick;
  }
  ...
  function pickText(pick) {
    return pick && pick.detail ? pick.text + ' — ' + pick.detail.value : (pick ? pick.text : '');
  }
  ...
  return {
    ...
    byId: byId, rankById: rankById, distText: distText, pickText: pickText,
