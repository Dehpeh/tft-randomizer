...
(function (root, factory) {
  ...
  function rollPlayer(opts) {
    ...
    for (const tier of plan) {
      ...
      picks.push({ id: res.pick.id, family: res.pick.family, text: res.pick.text, tier: tier, rerolls: res.rerolls });
    }
  ...
  function rerollSlot(picks, index, enabled, seed) {
    ...
    next[index] = { id: res.pick.id, family: res.pick.family, text: res.pick.text, tier: tier, rerolls: res.rerolls };
    return { ok: true, picks: next };
  }

  return {
    RANKS: RANKS, MINOR: MINOR, MAJOR: MAJOR, POOL: POOL, ALL: ALL,
    byId: byId, rankById: rankById, distText: distText,
