...
(function (root, factory) {
  ...
  const DEFAULTS = {
    ...
    detectAugments: true,    // rebuilt on the 'Choose One' template, and measured
    augmentMatch: 0.6,       // real screens scored 0.95-1.00, everything else under 0.36
    augmentHold: 2,          // samples above/below the line before it counts, to ignore a blip
  };
  ...
  /* How much this frame looks like an augment screen: -1 to 1. */
  ...
  function createDetector(options) {
    ...
    let above = 0;          // consecutive samples that look like an augment screen
    let below = 0;          // consecutive samples that do not
    let lastScore = 0;
    ...
    function push(frame, at) {
      ...
      /* --- the augment screen ---
         Matched on the words rather than the movement. Two samples above the
         line open it and two below close it, so a single mangled frame cannot
         invent or end one. */
      if (cfg.detectAugments) {
        const score = augmentScore(frame);
        lastScore = score;

        if (!inAugment) {
          above = score >= cfg.augmentMatch ? above + 1 : 0;
          if (above >= cfg.augmentHold) {
            inAugment = true;
            augmentSince = at;
            above = 0;
            below = 0;
            events.push({ kind: 'augment-open', at: at, score: +score.toFixed(2) });
          }
        } else {
          below = score < cfg.augmentMatch ? below + 1 : 0;
          if (below >= cfg.augmentHold) {
            inAugment = false;
            below = 0;
            events.push({
              kind: 'augment-take',
              ...
              /* Deliberately never named. Motion across the three cards at the
                 moment of choosing measured 33/33/33 on real footage: there is
                 no signal there. The screenshot goes to a human instead. */
              third: null,
              share: 0,
            });
          }
