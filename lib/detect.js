...
(function (root, factory) {
  ...
  const DIGITS = (typeof window !== 'undefined' && window.TFTDigits)
    || (typeof require === 'function' ? require('./digits.js') : null);

  /* A crop handed in already covers exactly its region, so the matcher reads
     the whole of it. */
  ...
  const DEFAULTS = {
    ...
    /* The round indicator, read rather than inferred. Every event carries the
       stage it happened in, which is what lets "5 costs banned until stage 5"
       and "roll to 0 gold at 4-2" be checked at all, and what stopped the
       augment stage being a guess based on counting. */
    watchStage: true,

    /* Whether the player did anything in a round, for the AFK rules.

       The first attempt at AFK measured how much the screen changed and called
       a still screen an idle player. That is not what it measures: TFT animates
       constantly, so it read the game rather than the person, and it was turned
       off. What actually distinguishes doing nothing is that nothing HAPPENS —
       no card leaves the shop, no reroll, and nothing moves on the board while
       the shop is up and it could have. Two of those the shop watcher already
       reports; the third is a frame difference over the board, but only during
       planning, because units move themselves in combat. */
    watchActivity: false,
    boardRegion: { x: 0.25, y: 0.20, w: 0.50, h: 0.50 },
    boardMoved: 6,          // mean abs difference that counts as something moving

    /* The 1-1 pet, watched without ever knowing which champion it is.

       The restriction says keep that unit on the bench all game. Identifying it
       would mean recognising a champion, and the only honest way to do that is
       a capture the player takes at the start of their own game — a step nobody
       will remember to do under pressure.

       It does not need identifying. A pet is the bench square that never
       changes: everything else on a bench is passing through, bought and sold
       and combined within a round or two. So the bench is watched for a square
       that has held the same thing for minutes, and the event is that square
       going empty. Which unit it was is on the clip. */
    watchBench: false,
    benchSettle: 150,       // seconds a square must hold before it counts as kept
  };
  ...
  function createDetector(options) {
    ...
    /* The stage has to hold still before it counts, the same as everything else
       here: the indicator is mid-animation on a round change and a single frame
    ...
    /* The last gold that could be read. The econ rule is about what gold was
       when a round ENDED, not what it passed through on the way. */
    let lastGold = null;
    let goldAtTurn = null;   // gold as the indicator first started to change

    /* What happened in the round currently on screen. */
    let acted = null;
    let boardPrev = null;

    /* Per bench square: whether something is on it, and since when. */
    const bench = [];
    ...
    /* `extra` carries readings the caller took at a resolution this frame does
       not have — the round indicator, chiefly. See readFromVideo in digits.js:
       the frame here is 320 wide because that is all the matchers need, and no
       digit survives that. */
    function push(frame, at, extra) {
      const events = [];
      const given = extra || {};
      ...
      if (cfg.detectAugments && M) {
        M.enabled().forEach((m) => {
          ...
          /* A matcher that reads digits gets a crop from the source rather than
             this frame, which is 320 wide because that is all the augment and
             shop work needs. Without it the gold matchers score against a
             counter four pixels tall and can never fire — which is exactly what
             they did in a live run, silently, while reporting as enabled.

             If no crop function was supplied the matcher is skipped rather than
             scored against pixels it cannot read, because a detector that
             quietly always says no is worse than one that is off. */
          let s;
          if (m.native) {
            if (!given.crop) return;
            const near = given.crop(m.region);
            if (!near) return;
            s = M.score(near, Object.assign({}, m, { region: WHOLE }));
          } else {
            s = M.score(frame, m);
          }
          st.score = s;
          ...
          if (!st.on) {
            ...
            /* The sixty seconds is an augment fact — real augments are stages
               apart, so a second one sooner is the same screen still being
               looked at. It is wrong for a matcher whose thing genuinely
               recurs: gold reaching 0 in 4-1 put gold-zero inside its cooldown
               and swallowed the 0 it reached in 4-2, so a player who did
               exactly what the rule asked was reported for not doing it. A
               matcher says how long its own thing takes to come round again. */
            const cooldown = m.cooldown !== undefined ? m.cooldown : cfg.augmentCooldown;
            if (st.above >= cfg.augmentHold && st.done && (at - st.done) < cooldown) {
              st.above = 0;
              st.on = true;
      ...
      /* --- which round is it ---
         Read first, so everything after it can be stamped with the answer. */
      /* The gold from BEFORE this tick. The round that just ended ended on
         that, not on whatever arrived with the new round's income — updating
         first and snapshotting second reported 35 for a round that ended on
         30. */
      const goldBefore = lastGold;
      if (given.gold !== undefined && given.gold !== null) lastGold = given.gold;

      if (cfg.watchStage && (DIGITS || given.stage !== undefined)) {
        const r = given.stage !== undefined
          ? { stage: given.stage, round: given.round }
          : DIGITS.readStage(frame, cfg.stageRegion);
        const key = r.stage === null ? null : r.stage + '-' + r.round;
        if (key && key === stagePending) {
          stageRun++;
          if (stageRun === 2 && key !== (stage === null ? null : stage + '-' + round)) {
            const wasStage = stage;

            /* The round just ended, so say what was done in it before moving
               on. Reported on the way out because that is the only point at
               which "nothing happened all round" is a fact rather than a
               guess. */
            if (acted) {
              events.push({
                kind: 'round-done', at: at,
                stage: acted.stage, round: acted.round,
                shopEvents: acted.shop, moved: acted.moved,
                idle: acted.shop === 0 && !acted.moved,
                seconds: Math.round(at - acted.since),
              });
            }
            stage = r.stage;
            round = r.round;
            acted = cfg.watchActivity ? { stage: stage, round: round, shop: 0, moved: false, since: at } : null;
            events.push({
              kind: 'stage', at: at, stage: stage, round: round, was: wasStage,
              /* What gold stood at as the round turned over. Watching gold
                 continuously catches every value it passes through during a
                 roll-down — measured on real footage, gold went 53, 50, 46, 46,
                 39, 30, 35 in five seconds, so 50 and 30 were each true for one
                 sample. The rule is about ending a round on a threshold, and
                 this is the only moment that answers it. */
              endedWith: goldAtTurn,
            });
            goldAtTurn = null;
          }
        } else {
          /* The moment the indicator first shows something new, the round that
             just ended is over — so the gold to remember is the one read before
             this tick, not the one two samples later when the change is
             confirmed and income has already landed. */
          if (key && stagePending) goldAtTurn = goldBefore;
          stagePending = key;
          stageRun = key ? 1 : 0;
        }
      }
      ...
      /* --- did anything move on the board ---
         Only while the shop is up. In combat the units move themselves, which
         is exactly the mistake the first AFK detector made. */
      if (acted && SHOP && !acted.moved) {
        const up = SHOP.visible(SHOP.read(frame, cfg.shopLayout));
        if (up) {
          if (boardPrev) {
            if (diff(boardPrev, frame, cfg.boardRegion) > cfg.boardMoved) acted.moved = true;
          }
          boardPrev = frame;
        } else {
          boardPrev = null;
      ...
      /* --- the bench ---
         A square that has held something for a long time and then goes empty is
         the pet leaving. Anything that has only been there a round or two is
         ordinary play and says nothing. */
      if (cfg.watchBench && FEAT) {
        for (let i = 0; i < 9; i++) {
          const here = FEAT.occupied(frame, FEAT.benchBox(i)).occupied;
          const was = bench[i] || (bench[i] = { on: false, since: at, run: 0 });

          if (here === was.on) { was.run += 1; continue; }
          /* Two samples before believing a square changed, so a unit being
             dragged over one does not read as it emptying. */
          if (was.run < 2) { was.run = 0; continue; }

          const held = at - was.since;
          if (was.on && !here && held >= cfg.benchSettle) {
            events.push({
              kind: 'bench-left', at: at, slot: i + 1,
              heldFor: Math.round(held),
            });
          }
          was.on = here;
          was.since = at;
      ...
      /* Stamp everything with the round it happened in. A note that says "a
         5-cost was bought" is a question; one that says "at 3-6" is an answer,
         because the ban lifts at stage 5. */
      events.forEach((e) => {
        if (e.kind !== 'stage' && stage !== null) { e.stage = stage; e.round = round; }
      });
