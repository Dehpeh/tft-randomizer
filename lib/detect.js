...
(function (root, factory) {
  ...
  function createDetector(options) {
    ...
    function slot(id) {
      if (!watch[id]) watch[id] = { on: false, above: 0, below: 0, since: 0, score: 0 };
      return watch[id];
    }
    ...
    function push(frame, at) {
      ...
      /* --- how many traits are lit ---
         Naming the banned trait would mean reading its name off the panel.
         Noticing that one more hexagon is lit than a moment ago does not, and
         the screenshot taken at that moment says which one it was. */
