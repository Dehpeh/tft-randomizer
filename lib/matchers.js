...
(function (root, factory) {
  ...
  const MATCHERS = [
    ...
    },
    {
      ...
      kind: 'gold-zero',
      /* Digits do not survive the 320-wide analysis frame — measured, gold
         reads 8/8 at full width and 0/8 at 720. So this one is scored against a
         crop taken from the source instead. See `native` in detect.js. */
      native: true,
      ...
      /* No cooldown. Every 0 is a fact the 4-2 rule is answered with, not a
         screen that might be double-counted — and a 0 in 4-1 is under a minute
         from a 0 in 4-2, so the default 60s hid the second one and accused a
         player who had complied. What this one says is rate-limited below;
         what it SEES must not be. */
      cooldown: 0,
      minGap: 45,
      max: 6,
      region: { x: 0.532, y: 0.8236, w: 0.026, h: 0.017 },
      template: 'built-in',
      evidence: 'Scored 8/8 on labelled gold values from a real 1080p game — 25, 29, 44, 42, 62, 46 rejected, both 10s caught.',
    },
    {
      id: 'traits-none',
    ...
    },
    {
      ...
      /* Same reason: gold sits on a ten many times in a game, and each is a
         fact rather than a repeat of the last one. */
      cooldown: 0,
      minGap: 20,
      max: 10,
      region: { x: 0.532, y: 0.8236, w: 0.026, h: 0.017 },
      template: 'built-in',
      evidence: 'Scored 8/8 on labelled gold from a real game. The earlier failure was the box including the coin icon, which counted as a third digit, not the resolution.',
    },
    {
      id: 'pet',
      label: 'The 1-1 pet',
    ...
    },
    {
      ...
      /* Declared, not built, and the reason is worth keeping.

         The idea was that three star pips are three small bright-gold blobs,
         level with each other and evenly spaced, so no picture would be needed.
         Run over a real game it fired on 16 of 75 samples — and a 3-star is
         permanent once it exists, so anything that comes and goes is wrong by
         construction. Looking at what it caught settled it: a striped plant, a
         wing, a patch of golden glow, a pink stretch of board. TFT's boards are
         full of small gold things in rows.

         It is not a tuning problem. Doing this properly needs the search
         narrowed to where pips can actually be — the board's hex centres, which
         are in perspective and would have to be measured — or a template
         captured from a real 3-star, which needs a game with one in it. Neither
         is hard; both need work this has not had.

         Until then it reports nothing, which is better than reporting scenery. */
      region: { x: 0.20, y: 0.25, w: 0.60, h: 0.45 },
      w: 24,
      h: 10,
      threshold: 0.75,
      template: null,
      evidence: 'Not built. The blob approach fired on 16 of 75 samples of one game, all of it board art — see the note in the source.',
    },
  ];
  ...
  function crop(frame, region, w, h) {
    ...
    /* Every source pixel in the cell contributes, rather than one being picked
       and the rest thrown away.

       This is not a refinement, it is the difference between working and not.
       Nearest-neighbour was fine on the 360p footage the template came from,
       where the augment region is about 128x29 going into a 64x14 template — a
       2x reduction that barely loses anything. On a 1440p game captured to
       1080p the same region is about 384x86, a 6x reduction, and taking one
       pixel in six off thin white lettering is a coin toss per stroke. Real
       augment screens scored 0.50-0.57 against a 0.60 threshold and the
       detector found nothing in an entire game.

       Averaging instead: the same template, the same frames, 0.93-0.97, with
       everything else still under 0.28. Every matcher crops through here, so
       they all get it. */
    const out = new Array(w * h);
    for (let ty = 0; ty < h; ty++) {
      const sy0 = y0 + Math.floor((ty * bh) / h);
      const sy1 = Math.max(sy0 + 1, y0 + Math.floor(((ty + 1) * bh) / h));
      for (let tx = 0; tx < w; tx++) {
        const sx0 = x0 + Math.floor((tx * bw) / w);
        const sx1 = Math.max(sx0 + 1, x0 + Math.floor(((tx + 1) * bw) / w));
        let sum = 0;
        let n = 0;
