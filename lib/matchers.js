...
(function (root, factory) {
  ...
  const MATCHERS = [
    ...
    },
    {
      ...
      /* The gold counter, bottom centre. A single "0" is all this needs to
         recognise — far easier than reading an arbitrary number, and it is
         exactly what the restriction asks. */
      region: { x: 0.532, y: 0.805, w: 0.020, h: 0.040 },
      w: 16,
      h: 20,
      threshold: 0.7,
      template: null,
      evidence: 'Not calibrated. Capture it with the gold counter showing a lone 0.',
    },
    {
      id: 'traits-none',
      label: 'No active traits',
      watches: 'Your game is Built Different (no active traits)',
      /* The trait panel down the left. Calibrate this one on a board with
         nothing activated: the matcher then asks whether the panel still looks
         like that. */
      ...
      threshold: 0.7,
      template: null,
      evidence: 'Not calibrated. Capture it on a board with no trait activated.',
    },
    {
      id: 'three-star',
      label: 'Three-star unit',
      watches: 'No 3-star unit allowed',
      /* The star pips sit above a unit, so unlike the others this one has no
         fixed home — a template here matches one board cell, and covering the
         board means running it across the grid. The honest note: this is the
         weakest of the five and the most likely to need a different approach. */
      region: { x: 0.30, y: 0.35, w: 0.40, h: 0.30 },
      w: 24,
      h: 10,
      threshold: 0.75,
      template: null,
      evidence: 'Not calibrated, and the hardest: pips move with the unit, so one template covers one place on the board.',
    },
  ];
