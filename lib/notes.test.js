...
(function (root, factory) {
  ...
  /* The end-of-game rows rather than the feed. Shaped like notes so the same
     has() can read them. */
  function tallyOf(picks, events) {
    const N = dep('TFTNotes', './notes.js');
    const M = dep('TFTMatchers', './matchers.js');
    const book = N.createNotebook({ rules: N.rulesFrom(picks, (p) => p.details || []), matchers: M });
    events.forEach((e) => book.push(e));
    return book.summary().map((r) => ({ note: r.text }));
  }
  ...
  /* The major: lock it on every reroll, not once a stage. */
  const lockAlways = (slot) => ({ id: 'mj-shoplock', details: [{ label: 'Shop slot', value: slot + 'th slot' }] });
  ...
  function run(log) {
    ...
    has('an augment note names the round it read',
      play([augment('2-1', 'left')], [{ kind: 'augment-take', at: 10, openFor: 20, stage: 2, round: 1 }]), '2-1 · your roll said left');
    has('and quotes nothing when the roll was for another stage',
      play([augment('2-1', 'left')], [{ kind: 'augment-take', at: 10, openFor: 20, stage: 4, round: 2 }]), '4-2 · nothing rolled');
    has('a capture too small to read says so rather than guessing',
      play([augment('2-1', 'left')], [{ kind: 'augment-take', at: 10, openFor: 20 }]),
      'counted, the round indicator was not readable');
    ...
    /* The counter used to decide which stage an augment belonged to, so a
       detector reporting one screen twice walked it past three and mislabelled
       everything after. Reading the indicator means a repeat lands on the stage
       it is actually on. */
    const twice = play([augment('2-1', 'left')], [
      { kind: 'augment-take', at: 30, openFor: 20, stage: 2, round: 1 },
      { kind: 'augment-take', at: 90, openFor: 8, stage: 2, round: 1 },
    ]);
    is('a repeated screen stays on the stage it is actually on',
      twice.filter((n) => n.note.indexOf('2-1') > -1).length, 2);
    ...
    say('the shop rules');
    /* Two rules that read alike and are not. The major says lock a shop space,
       so any reroll without it is a breach. The minor says lock one "1 round
       every stage", so a reroll without it is ordinary play and only a whole
       stage passing without it is wrong. Treating them alike put a note on
       nearly every reroll of a game. */
    const majorLock = { id: 'mj-shoplock', details: [{ label: 'Shop slot', value: '3rd slot' }] };
    const reroll = (at, kept) => ({ kind: 'shop-reroll', at: at, kept: kept, offered: [1, 1, 1, 1, 1], wasOffered: [1, 1, 1, 1, 1] });

    has('the major flags a single reroll that did not hold the slot',
      play([majorLock], [reroll(10, [1])]), 'slot 3 did not hold');
    is('and says nothing when it did hold', play([majorLock], [reroll(10, [3])]).length, 0);

    is('the minor says nothing about one reroll without the lock',
      play([lock(3)], [{ kind: 'stage', at: 0, stage: 2, round: 1 }, reroll(10, [])]).length, 0);
    is('nor about a stage where the lock was used once',
      play([lock(3)], [{ kind: 'stage', at: 0, stage: 2, round: 1 }, reroll(10, []), reroll(40, [3]),
        reroll(70, []), { kind: 'stage', at: 100, stage: 3, round: 1 }]).length, 0);
    has('but flags a stage that went by without it ever holding',
      play([lock(3)], [{ kind: 'stage', at: 0, stage: 2, round: 1 }, reroll(10, []), reroll(40, []),
        { kind: 'stage', at: 100, stage: 3, round: 1 }]), 'Stage 2 went by without slot 3');
    ...
    say('one note per decision, not one per moment');
    const augRule = [{ id: 'mj-augment', details: [
      { label: '2-1', value: 'left' }, { label: '3-2', value: 'right' }, { label: '4-2', value: 'left' }] }];
    const screens = [];
    [[2, 1], [3, 2], [4, 2]].forEach(function (pair, i) {
      screens.push({ kind: 'augment-open', at: i * 300, stage: pair[0], round: pair[1] });
      screens.push({ kind: 'augment-take', at: i * 300 + 20, openFor: 20, stage: pair[0], round: pair[1] });
    });
    is('three augment screens make three notes, not six', play(augRule, screens).length, 3);

    /* A whole stage sat out is one obligation, not one per round. Left alone it
       wrote a note for every round of that stage, all saying the same thing. */
    const wholeStage = [{ id: 'mj-afk', details: [{ label: 'Stage', value: 'stage 4' }] }];
    const busy = [];
    for (let r = 1; r <= 7; r++) {
      busy.push({ kind: 'round-done', at: r * 40, stage: 4, round: r, shopEvents: 2, moved: true, idle: false });
    }
    is('a whole stage played through makes one note, not seven', play(wholeStage, busy).length, 1);
    has('and it says it was the whole stage', play(wholeStage, busy), 'the whole of stage 4');

    /* A state is counted, not repeated. Twelve banned purchases used to make
       nine identical lines and then one saying counting had stopped — which
       tells a gamemaster where the cap sat, not how many times it happened. */
    say('a state is said once and counted in full');
    const many = [];
    for (let i = 0; i < 12; i++) many.push({ kind: 'shop-buy', at: i * 40, slot: 1, cost: 5, stage: 3, round: 2 });
    is('twelve banned buys make one note, not nine', play([costban], many).length, 1);
    has('and the row carries the real number, not the cap',
      tallyOf([costban], many), 'Bought a banned cost 12 times');

    /* The one that started this: a player who is not locking trips the rule on
       every reroll they make, which is dozens. */
    const rolls = [];
    for (let i = 0; i < 60; i++) rolls.push(reroll(i * 25, []));
    is('sixty rerolls without the lock make one note, not nine',
      play([lockAlways(3)], rolls).length, 1);
    has('and the row says how often it actually held',
      tallyOf([lockAlways(3)], rolls), 'Slot 3 held on 0 of 60 rerolls');
    has('a player who did lock it gets a row saying so',
      tallyOf([lockAlways(3)], [reroll(10, [3]), reroll(40, [3])]), 'Slot 3 held on 2 of 2 rerolls');
