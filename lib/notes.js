...
(function (root, factory) {
  ...
  /* The nth augment screen of a game is 2-1, then 3-2, then 4-2. Counting them
     is enough to know which restriction, if any, is in play — which is only
     true because the detector now reports one screen once. When it flapped,
     this counter raced past three and pinned everything to 4-2. */
  ...
  function createNotebook(options) {
    ...
    function stageNote() {
      const stage = STAGES[Math.min(STAGES.length - 1, augmentSeen)];
    ...
    function push(e) {
      ...
      if (e.kind === 'augment-open') note('augment', 'Augment screen' + stageNote());

      if (e.kind === 'augment-take') {
        note('augment', 'Augment taken after ' + e.openFor + 's — check the shot' + stageNote());
        augmentSeen += 1;
      }
      ...
      if (e.kind === 'shop-buy') {
        const banGate = e.cost !== null && rules.bannedCosts.indexOf(e.cost) !== -1
          && allow('costban', e.at, 20, 8);
        if (banGate) {
          note('note', 'Bought a ' + e.cost + '-cost from slot ' + e.slot
            + ' — check the stage, the ban lifts at stage 5' + capped(banGate));
        }
      }
