...
(function (root, factory) {
  ...
  /* Which restriction each matcher exists for, and when it is worth saying.
     ...
     otherwise. */
  /* The econ thresholds a round can end on. 100 is not one of them — the rule
     names the ten-gold marks. */
  const ECON_MARKS = [10, 20, 30, 40, 50];
  ...
  const MATCHER_RULES = {
    ...
    /* Tracked rather than reported. Gold passing through a round number during
       a roll-down is not the rule; ending a round on one is, and that is
       written when the round turns over. */
    'gold-threshold': { needs: 'econ', silent: true },
  ...
  function rulesFrom(picks, detailsOf) {
    ...
    (picks || []).forEach((p) => {
      ...
      /* Two different obligations wearing similar words. The major says lock a
         shop space, full stop, so every reroll that did not hold it is a
         breach. The minor says lock one "1 round every stage" — the player
         chooses which round, so a reroll without the lock is ordinary play and
         the only thing that can be wrong is a whole stage going by without the
         slot ever being held. Treating them the same put a note on nearly every
         reroll of the game. */
      if (p.id === 'mn-shoplock') { out.lockSlot = slotOf('Shop slot'); out.lockOncePerStage = true; }
      if (p.id === 'mj-shoplock') { out.lockSlot = slotOf('Shop slot'); out.lockAlways = true; }
  ...
  function createNotebook(options) {
    ...
    let sawLock = false;   // the locked slot held through a reroll this stage
    let lastRound = null;
    const seen = {};
    ...
    /* --- states and moments ---
       A moment is a decision: an augment taken, a banned unit bought, a trait
       lit. It earns a line on the feed and a clip, because a person has to look
       at it and judge.

       A state is a condition that either holds for the game or does not —
       whether the slot you were told to lock is being locked. Reported once per
       moment it was seen, one state produced nine identical lines and then
       announced that it had stopped counting. Eight players of that is a stream,
       not a scoreboard, and the gamemaster still does not know the answer: nine
       is where the cap sat, not how many times it happened.

       So a state is counted every time it is seen, said once — the first time,
       which is the one that carries the clip — and handed over at the end as a
       single row with the real number in it. Nine lines become one line and one
       count, and the count is complete instead of truncated. */
    const tally = {};
    function saw(key, breach) {
      const t = tally[key] || (tally[key] = { breach: 0, of: 0 });
      t.of += 1;
      if (breach) t.breach += 1;
      return t;
    }
    /* Only the first breach of a state speaks. The rest are counted. */
    const firstOf = (key) => (tally[key] || {}).breach === 1;
    ...
    function push(e) {
      ...
      if (e.kind === 'stage') {
        ...
        /* A stage went by without the slot ever being held, which is the whole
           of what the minor lock rule asks for. Checked when the stage number
           turns over, because that is the first moment it can be answered. */
        if (rules.lockOncePerStage && rules.lockSlot && leaving && e.stage !== leaving.stage) {
          saw('lockstage', !sawLock);
        }
        if (rules.lockOncePerStage && rules.lockSlot && leaving
          && e.stage !== leaving.stage && !sawLock
          && allow('lockstage', e.at, 15, 6)) {
          note('note', 'Stage ' + leaving.stage + ' went by without slot ' + rules.lockSlot
            + ' ever being held through a reroll');
        }
        if (leaving && e.stage !== leaving.stage) sawLock = false;

        /* Ended a round sitting on a ten. */
        if (rules.econ && leaving && e.endedWith !== null && e.endedWith !== undefined) {
          saw('econ', ECON_MARKS.indexOf(e.endedWith) !== -1);
        }
        if (rules.econ && leaving && ECON_MARKS.indexOf(e.endedWith) !== -1
          && firstOf('econ')) {
          note('note', 'Ended ' + leaving.stage + '-' + leaving.round + ' on ' + e.endedWith
            + ' gold — the rule asks for a roll when a round ends on a ten');
        }
      ...
      /* One note per augment decision. It used to write two — the screen
         opening and the choice being made — which is six notes a game for a
         rule that is decided three times.

         The one worth keeping is the take: its clip runs from five seconds
         before the choice to three after, so the cards, the cursor and the
         click are all in it. The opening note carried the same cards and no
         decision. */
      ...
      if (e.kind === 'bench-left' && rules.pet) saw('pet', true);
      if (e.kind === 'bench-left' && rules.pet && firstOf('pet')) {
      ...
      if (e.kind === 'round-done' && e.stage) {
        const sitRound = rules.afkRound && e.round === rules.afkRound;
        const sitStage = rules.afkStage && e.stage === rules.afkStage;

        /* "AFK for a whole stage" is one obligation, not one per round. Left
           alone it wrote a note for every round of that stage — six or seven
           saying the same thing. One is enough to send a gamemaster to the
           clip, so the key is the stage rather than the moment. */
        const key = sitStage ? 'afk:stage' + e.stage : 'afk';
        const gap = sitStage ? 1e9 : 15;

        if ((sitRound || sitStage) && !e.idle && allow(key, e.at, gap, 8)) {
          ...
          note('note', sitStage
            ? 'Was meant to sit out the whole of stage ' + e.stage + ' — ' + bits.join(', ')
              + ' at ' + e.stage + '-' + e.round + '. The clip shows it.'
            : 'Was meant to sit out ' + e.stage + '-' + e.round + ' — ' + bits.join(', ')
              + '. The clip shows it.');
        }
      }
      ...
      if (e.kind === 'shop-reroll') {
        ...
        const held = rules.lockSlot && (e.kept || []).indexOf(rules.lockSlot) !== -1;
        if (held) sawLock = true;

        /* Only the major complains about a single reroll. */
        if (rules.lockAlways && rules.lockSlot) {
          saw('lock', !held);
          if (!held && firstOf('lock')) {
        ...
        if (rules.mustBuy.length) {
          ...
          saw('forcebuy', passed.length > 0);
          if (passed.length && firstOf('forcebuy')) {
            ...
            note('note', 'Shop changed with ' + said + ' still in it');
          }
        }
      }
      ...
      if (e.kind === 'shop-buy') {
        ...
        const banned = e.cost !== null && rules.bannedCosts.indexOf(e.cost) !== -1 && !lifted;
        if (rules.bannedCosts.length && !lifted) saw('costban', banned);
        if (banned && firstOf('costban')) {
          const where = e.stage ? ' at ' + e.stage + '-' + e.round : ' (stage unreadable)';
          note('note', 'Bought a ' + e.cost + '-cost from slot ' + e.slot + where
            + ', and the ban runs until stage 5');
        }
      }

      if (e.kind === 'traits-up' && (rules.traitBan || rules.builtDifferent)) {
        saw('traits', true);
        if (firstOf('traits')) {
    ...
    /* What each counted state reads as at the end of a game. The denominator is
       the point: "did not hold" is an accusation, "held on 2 of 47" is a fact a
       gamemaster can act on without watching anything. A row with nothing in it
       is still worth sending — "held every time" is the answer to the question
       the restriction asked, and silence is not. */
    const STATE_TEXT = {
      lock: (t) => 'Slot ' + rules.lockSlot + ' held on ' + (t.of - t.breach) + ' of ' + t.of + ' rerolls',
      lockstage: (t) => 'The slot was locked in ' + (t.of - t.breach) + ' of ' + t.of + ' stages',
      forcebuy: (t) => 'A must-buy was left in the shop ' + t.breach + ' of ' + t.of + ' rerolls',
      costban: (t) => (t.breach === 0 ? 'No banned cost bought before stage 5'
        : 'Bought a banned cost ' + t.breach + (t.breach === 1 ? ' time' : ' times') + ' before stage 5'),
      econ: (t) => 'Ended ' + t.breach + ' of ' + t.of + ' rounds on a ten',
      traits: (t) => (t.breach === 0 ? 'No banned trait activated'
        : 'A banned trait activated ' + t.breach + (t.breach === 1 ? ' time' : ' times')),
      pet: (t) => (t.breach === 0 ? 'The pet stayed on its square'
        : 'The pet left its square ' + t.breach + (t.breach === 1 ? ' time' : ' times')),
    };

    /* One row per state the player actually had a rule about, with the real
       count rather than wherever a cap happened to fall. */
    function summary() {
      return Object.keys(tally).filter((k) => STATE_TEXT[k]).map((k) => ({
        key: k,
        text: STATE_TEXT[k](tally[k]),
        breach: tally[k].breach > 0,
        count: tally[k].breach,
        of: tally[k].of,
      }));
    }
